...
import net.opentsdb.tsd.ZkClientWorker;
...
final class TSDMain {
  ...
  public static void main(String[] args) throws IOException {
    ...
    System.out.println("------------------"+config.getString("tsd.http.staticroot"));
    ...
    try {
     ...
     if(config.enable_monitor()){
         String uidTable = config.getString("tsd.storage.hbase.uid_table");
         if(null == uidTable || "".equals(uidTable)) uidTable = "tsdb-uid";
         System.out.println("----------uidTable------------" + uidTable);
         UniqueId uid = new UniqueId(tsdb.getClient(), uidTable.getBytes(), "metrics", 3);
         try {
             uid.getId("write.count");
         } catch (NoSuchUniqueName e) {
             PutMetricsDataPoint putMetricsDataPoint = new PutMetricsDataPoint(tsdb, true);
             putMetricsDataPoint.putRawMetrics("write.count", 0);
             uid.getOrCreateId("write.request");
             uid.getOrCreateId("read.request");
         }
         //work node monitor and do the aggration job
         ZkClientWorker zkClient = new ZkClientWorker(tsdb);
         zkClient.execute();
     }
    } catch (Throwable e) {
