...
public final class RpcManager {
  ...
  private void initializeBuiltinRpcs(final OperationMode mode,
    ...
    //final Boolean enableAuth = tsdb.getConfig().getString("tsd.core.authentication.enable").equals("true");
    final Boolean enableAuth = tsdb.getConfig().getBoolean("tsd.core.authentication.enable");
    ...
    final Boolean enableMonitor = tsdb.getConfig().getBoolean("tsd.metrics.monitor.enable");
    
    LOG.info("Mode: {}, HTTP UI Enabled: {}, HTTP API Enabled: {}, enableAuth:{}", mode, enableUi, enableApi, enableAuth);
    ...
    case WRITEONLY:
      ...
      if (enableApi) {
        ...
        //lynn add api
        if(enableAuth){
          LOG.debug("------register user url----api/user/put,api/user/del");
          http.put("api/user/put", new PutUserRpc());
          http.put("api/user/del", new DeleteUserRpc());
        }
      }
      break;
    case READONLY:
      ...
      if (enableApi) {
        ...
        //lynn add api
        if(enableAuth){
          LOG.debug("------register user url----api/user/query");
          http.put("api/user/query", new QueryUserRpc());
        }
        
        //201809 add by wf
        if(enableMonitor){
        	LOG.debug("------register metric url----api/metrics/query");
            http.put("api/metrics/query", new QueryMetricsRpc());
        }
      }
    ...
    case READWRITE:
      ...
      if (enableApi) {
        ...
        //lynn add api
        if(enableAuth){
          LOG.debug("------register user url----api/user/put,api/user/query,api/user/del");
          http.put("api/user/put", new PutUserRpc());
          http.put("api/user/del", new DeleteUserRpc());
          http.put("api/user/query", new QueryUserRpc());
          LOG.debug("------registered user url----api/user/put,api/user/query,api/user/del");
        }
        //201809 add by wf
        if(enableMonitor){
        	LOG.debug("------register metric url----api/metrics/query");
    ...
    if(enableAuth){
      LOG.debug("------register user url----api/auth/token");
      http.put("api/auth/token", new TokenRpc());
    }
    LOG.debug("URL_api/user/query:{} ", http.build().containsKey("api/user/query"));
    LOG.debug("URL_api/user/put: {}", http.build().containsKey("api/user/put"));
    LOG.debug("URL_api/user/del: {}", http.build().containsKey("api/user/del"));
    LOG.debug("URL_api/auth/token: {}", http.build().containsKey("api/auth/token"));
    LOG.debug("URL_api/metrics/query: {}", http.build().containsKey("api/metrics/query"));
    if (enableDieDieDie) {
