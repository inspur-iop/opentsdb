...
/**
 * minutely job for aggregate write.count/write.request/read.request datapoint
...
public class MetricMinutelyAggregation implements Job {
	private TSDB tsdb;
    ...
    private String minutelyMetricInfoZnode;
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        //get the start time in minute
        startTime = Long.parseLong(startTimeStr);
        if ((startTime & Const.SECOND_MASK) != 0) {
        	startTime = (startTime/1000) - ((startTime/1000)%60);
        }else {
        	startTime = startTime -(startTime%60);
        }
               
        QueryRawMetrics queryRawMetrics = new QueryRawMetrics(tsdb, zkClient, startTime, "1m-sum");
	...
	public String  getStartTime(){
	    ...
	    try {
			if (null != zkClient.checkExists().forPath(minutelyMetricInfoZnode)) {
			    String tmpStr = new String(zkClient.getData().forPath(minutelyMetricInfoZnode));
		...
		if (startTime == null || startTime.isEmpty()) {
			...
			startTimetmp = startTimetmp - (startTimetmp%60) - 60;
