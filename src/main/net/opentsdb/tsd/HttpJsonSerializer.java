...
class HttpJsonSerializer extends HttpSerializer {
  ...
  static TypeReference<ArrayList<User>> TR_DB_USER =
   ...
   */
  @Override
  public List<User> parseUserPutV1() {
    if (!query.hasContent()) {
      throw new BadRequestException("Missing request content");
    }
    ...
    try {
      if (firstbyte == '{') {
        final User user =
                JSON.parseToObject(content, User.class);
        final ArrayList<User> users =
                new ArrayList<User>(1);
        users.add(user);
        return users;
      } else {
        return JSON.parseToObject(content, TR_DB_USER);
      }
    } catch (IllegalArgumentException iae) {
      throw new BadRequestException("Unable to parse the given JSON", iae);
    }
   ...
   */
  @Override
  public <T extends User> List<T> parseUserPutV1(
          final Class<T> type, final TypeReference<ArrayList<T>> typeReference) {
    if (!query.hasContent()) {
      throw new BadRequestException("Missing request content");
    }
