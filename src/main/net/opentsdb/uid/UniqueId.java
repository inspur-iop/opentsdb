...
//    final String key = fromBytes(id);
    final String key = new String(id, Charset.forName("ISO8859-1"));
    ...
    if (found != null && !found.equals(name)) {
      byte[] idf = lru_name_cache.getIfPresent(found);
      throw new IllegalStateException("id=" + Arrays.toString(id) + " => name="
          + name + ", already mapped to " + found + ", id-found = " + Arrays.toString(idf));
    }
  }
