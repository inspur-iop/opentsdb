...
final class TsdbQuery implements Query {
  ...
  //201809 add by wf
  @Override
  public Deferred<DataPoints[]> runAsyncForMetric(String flg) throws HBaseException {
    Deferred<DataPoints[]> result = null;
    result = findSpansForMetric(flg).addCallback(new GroupByAndAggregateCB());   
    return result;
  }
  ...
  /**
   * copy the function findSpans()
   * do the same thing except get scanner's function is different
   * because of query the different table
   * function  getScanner query tsdb
   * function getScannerForMetirc query tsdb-raw-metrics
   * 201809 add by wf
   */
  private Deferred<TreeMap<byte[], Span>> findSpansForMetric(String flg) throws HBaseException {
    final short metric_width = tsdb.metrics.width();
    final TreeMap<byte[], Span> spans = // The key is a row key from HBase.
      new TreeMap<byte[], Span>(new SpanCmp(
          (short)(Const.SALT_WIDTH() + metric_width)));
    ...
    if (Const.SALT_WIDTH() > 0) {
      ...
      final List<Scanner> scanners = new ArrayList<Scanner>(1);
      scanners.add(getScannerForMetirc(0, flg));//the changed point
      scan_start_time = DateTime.nanoTime();
      return new SaltScanner(tsdb, metric, scanners, spans, scanner_filters,
          delete, rollup_query, query_stats, query_index, null, max_bytes, 
          max_data_points).scan();
...
//               downsampler = rollup_query.getRollupAgg();
  ...
  /**
   * copy the function getScanner(final int salt_bucket)
   * do the same thing except different table and metric to query
   * function  getScanner query tsdb
   * function getScannerForMetirc query tsdb-raw-metrics
   * 201809 add by wf
   */
  protected Scanner getScannerForMetirc(final int salt_bucket, String flg) throws HBaseException {
    final short metric_width = tsdb.metrics.width();
    ...
    final Scanner scanner = QueryUtil.getMetricScanner(tsdb, salt_bucket, metric, 
        ...
        ? -1  // Will scan until the end (0xFFF...).
        : (int) getScanEndTimeSeconds(), 
        getMetricTableName(flg), //the point changed
        TSDB.RAWMETRICSFAMILY());//the point changed
    if(tsdb.getConfig().use_otsdb_timestamp()) {
      long stTime = (getScanStartTimeSeconds() * 1000);
      long endTime = end_time == UNSET ? -1 : (getScanEndTimeSeconds() * 1000);
      if (tsdb.getConfig().get_date_tiered_compaction_start() <= stTime) {
  ...
  /**
   * get the table name of store metric datapoint 
   * @param query table flag
   * @return table name as byte array
   * 201809 add by wf
   */
 //
  private byte[] getMetricTableName(String flg) {
	  if (flg.equals("s")) {
		  return tsdb.secondlyMetricsTable();
      }else if (flg.equals("m")) {
    	  return tsdb.minutelyMetricsTable();
      }else if (flg.equals("h")) {
          return tsdb.hourlyMetricsTable();
      }else {
          return tsdb.rawMetricsTable();
      }
  }
