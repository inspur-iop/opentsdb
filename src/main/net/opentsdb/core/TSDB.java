...
public final class TSDB {
  ...
  static final byte[] RAWMETRICSFAMILY = { 'c' };//201809 add by wf
  ...
  /** Name of the table  which  metrics datapoint are stored.  201809 add by wf*/
  final byte[] raw_metrics_table;
  final byte[] secondly_metrics_table;
  final byte[] minutely_metrics_table;
  final byte[] hourly_metrics_table;
  ...
  public TSDB(final HBaseClient client, final Config config) {
    ...
    raw_metrics_table = config.getString("tsd.storage.hbase.metrics_table").getBytes(CHARSET);
    secondly_metrics_table = config.getString("tsd.storage.hbase.metrics_table_sencondly").getBytes(CHARSET);
    minutely_metrics_table = config.getString("tsd.storage.hbase.metrics_table_minutely").getBytes(CHARSET);
    hourly_metrics_table = config.getString("tsd.storage.hbase.metrics_table_hourly").getBytes(CHARSET);
  ...
  /** @return The data point column family name */
  public static byte[] RAWMETRICSFAMILY() {
    return RAWMETRICSFAMILY;
  }
  ...
  public Deferred<ArrayList<Object>> checkNecessaryTablesExist() {
    ...
    if (config.enable_monitor()) {
      checks.add(client.ensureTableExists(
    	  config.getString("tsd.storage.hbase.metrics_table")));
      checks.add(client.ensureTableExists(
          config.getString("tsd.storage.hbase.metrics_table_sencondly")));
      checks.add(client.ensureTableExists(
          config.getString("tsd.storage.hbase.metrics_table_minutely")));
      checks.add(client.ensureTableExists(
          config.getString("tsd.storage.hbase.metrics_table_hourly")));
    }
    if (config.enable_auth()) {
      checks.add(client.ensureTableExists(
          config.getString("tsd.core.user.table")));
    }
  ...
  /** @return the name of the meta table as a byte array for client requests */
  public byte[] rawMetricsTable() {
    return this.raw_metrics_table;
  }
  public byte[] secondlyMetricsTable() {
	    return this.secondly_metrics_table;
  }
  public byte[] minutelyMetricsTable() {
	    return this.minutely_metrics_table;
  }
  public byte[] hourlyMetricsTable() {
	    return this.hourly_metrics_table;
  }
  /**
