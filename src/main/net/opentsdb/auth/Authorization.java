...
public abstract class Authorization {
  ...
  /**
   *Determines if the user is allowed to access the given api.
   * The returned state contains a status code regarding whether or not the access
   * is allowed. If the access IS allowed, the same user state passed as an
   * argument maybe returned.
   * @param state
   * @param httpRequest A non-null query.
   * @return An AuthState object with a valid AuthStatus to evaluate for
   */
  public abstract AuthState allowHttpApi(final AuthState state, final HttpRequest httpRequest);
